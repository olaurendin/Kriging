...
def _find_statistics(X, y, variogram_function,variogram_model_parameters,
    coordinates_type, dist_max = np.inf):
    ...
    from pykrige.variogram_models import spherical_variogram_model, gaussian_variogram_model, exponential_variogram_model
    from time import time

    delta = np.zeros(y.shape)
    sigma = np.zeros(y.shape)

    # Randomize the data in case there is a link between the training points
    # np.random.seed(42)
    # np.random.shuffle(X)
    # np.random.seed(42)
    # np.random.shuffle(y)

    vmp = variogram_model_parameters    # Just for clarity of the code
    thresh = 0.99

    if variogram_function.__name__ == "spherical_variogram_model":
        eff_range=vmp[1]
    elif variogram_function.__name__ in ["gaussian_variogram_model", "exponential_variogram_model"]:
        # v = [psill, range, nugget]
        if isinstance(vmp, list):
            v = np.array([[vmp[0]], [vmp[1]], [vmp[2]]])
            eff_range = sc.optimize.fsolve(lambda x: variogram_function(v, x) - thresh*(vmp[0]+vmp[2]), vmp[1])[0]
        else:
            print("variogram model parameters must be a list")
    else:   # power_variogram_model, hole_effect_variogram_model, linear_variogram_model or custom
        eff_range = dist_max
    eff_range=min(eff_range,dist_max)
    if eff_range == dist_max:
        print("distance max is lower than theoretical effective range for a {}%\
        threshold, oddities may occur.".format(thresh) )

    tree = cKDTree(X)
    neighbors = tree.query_ball_tree(tree,eff_range)#,eps=0.0)
    ...
    for i in range(y.shape[0]):
        inds = neighbors[i]
        k, ss = _krige(X[inds, :], y[inds], X[i, :], variogram_function,
                       variogram_model_parameters, coordinates_type)
        ...
        if np.absolute(ss) < eps:
            print("error points too close : {}, {}".format(ss, eps))
            continue

        delta[i] = y[i] - k
        sigma[i] = np.sqrt(ss)
    time1 = time()
    print("process finished in {:5f}s".format(time1-time0))
